...
void Command_parser::parse_command_type(Command &command) const
{
    ...
    else if (!strcmp(buf, "set_pm_controller_mode"))
        command.type = Command::Type::set_pm_controller_mode;
    ...
    else if (!strcmp(buf, "test_matrix_write_read"))
        command.type = Command::Type::test_matrix_write_read;
    else
